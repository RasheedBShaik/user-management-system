...
export default function UserPage() {
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      showToast("error", err.response?.data?.msg || "Update failed (Access Denied)");
    } finally {
      setUpdating(false);
    }
  };
  ...
  return (
    <div className=" bg-[#0a0a0a] text-white selection:bg-blue-500/30 pb-20">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 flex items-center gap-2">
                ...
                Logged in as <span className="text-gray-200 font-semibold capitalize">{user?.name}</span>
              </p>
            </div>
          </div>
          ...
          <aside className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-linear-to-br from-blue-600/20 to-purple-600/20 border border-white/5 rounded-3xl p-8 relative overflow-hidden group">
              <div className="relative z-10">
                ...
                <h3 className="text-2xl font-bold mb-1 capitalize">{user?.name}</h3>
                <p className="text-gray-400 text-sm mb-1">{user?.email}</p>
                <p className="text-gray-400 text-sm mb-6">{user?.phone || "No phone added"}</p>
                ...
                <div className="space-y-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input
                        className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl focus:ring-2 focus:ring-blue-500/50 outline-none transition-all hover:bg-white/[0.07] capitalize"
                        value={name}
                    ...
                    <div className="md:col-span-2">
                      ...
                      <input
                        className="w-full bg-white/2 border border-white/5 p-4 rounded-2xl text-gray-500 cursor-not-allowed"
                        value={user?.email}
                        disabled
                      />
                    </div>
                ...
                return (
                  <div key={idx} className="flex items-center justify-between p-4 bg-white/2 border border-white/5 rounded-2xl group/member hover:bg-white/4 transition-colors">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-gray-200 capitalize">{member.member?.name || "System User"}</p>
                    ...
                    <div className="text-right flex flex-col items-end gap-2">
                      {isEditing ? (
                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-2">
                          ...
                          />
                          <button onClick={() => handleUpdateModule(mId)} className="p-1.5 bg-blue-600 rounded-lg hover:bg-blue-500">
                            {isSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
                          </button>
                          <button onClick={() => setEditingId(null)} className="p-1.5 bg-white/10 rounded-lg"><X size={14} /></button>
                        </div>
                      ) : (
                        <>
                          ...
                          {isLeadOfProject && (
                            <button
                              className="text-[10px] flex items-center gap-1 text-gray-400 hover:text-white cursor-pointer group-hover/member:text-blue-400"
                              onClick={() => { setEditingId(mId); setNewModule(member.module); }}
                            >
